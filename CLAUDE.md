...
依赖：本地 ComfyUI 服务（端口 8188）+ 已下载的 GGUF 模型。
```bash
# 启动 ComfyUI
...
```
每个成功生成的文件旁会写 `.zimage_done` 标记，支持断点续跑。提示词复用 `generate_illustrations_flux.py` 的 `DEMON_VISUALS` 字典（zh 字段），没有提示词的妖怪用通用 fallback 提示词。`--lowvram` 是 8GB 显存 RTX 4060 上的安全配置。
...
3. **JS** (lines 237–465): vanilla, no ESM imports. Four functions drive the app:
   - `init()` — `fetch('tribulations.json')`, then `buildMap()` + `bindScroll()`
   ...
   - `openPanel(idx)` — populates `#p-*` slots; if `demons[].name` matches `illustrations/<name>.png`, image is embedded with `onerror` placeholder
...
- **All demon illustrations must be 古风 (classical Chinese) Tang-dynasty style** — prompts in both `generate_illustrations_flux.py` (legacy) and `generate_illustrations_zimage.py` (active) enforce this with keywords like "唐代壁画", "工笔重彩", "绢本设色". When adding a new demon prompt, match that style vocabulary.
...
No build step. The `.gitignore` keeps dev artifacts (screenshots, `.playwright-mcp/`, `map-bg*.png`, regeneration markers) and the large source text out of the repo.
